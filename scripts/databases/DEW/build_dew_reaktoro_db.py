...
def build_yaml_for_excel(xls_path: str, tag: str, out_dir: Path = None):
    """
    ...
    out_dir: directory where YAML files are written (defaults to CWD).
    """
    ...
    aq_out = out_dir / f"{tag}-aqueous.yaml"
    gas_out = out_dir / f"{tag}-gas.yaml"
...
if __name__ == "__main__":
    ...
    # Excel source files live in embedded/databases/DEW/ (3 levels up from scripts/databases/DEW/).
    _HERE = Path(__file__).parent
    _DB_DIR = (_HERE / "../../../embedded/databases/DEW").resolve()

    build_yaml_for_excel(
        _DB_DIR / "Latest_DEW2024.xlsm", tag="dew2024", out_dir=_DB_DIR
    )
    build_yaml_for_excel(_DB_DIR / "DEW_2019.xlsm", tag="dew2019", out_dir=_DB_DIR)

...
# Keep Zn minerals while allowing key gangue proxies to appear/disappear.
...
# Add Fe/Ca/Mg aqueous species required for gangue buffering interactions.
extra_aq = [
    "Ca+2",
    "Ca(OH)+",
    "CaCO3,aq",
...
# Seed candidate solids to let assemblages stabilize if favorable.
...
solved = 0
total = len(Tvals) * len(pHvals) * len(logfO2_vals) * len(logfCO2_vals)

for T in Tvals:
    # Reuse a state through the local T-sweep branch for better convergence.
    ...
    for pH in pHvals:
        for lfO2 in logfO2_vals:
            for lfCO2 in logfCO2_vals:
                ...
                solved += 1
                point = (float(T), float(pH), float(lfO2), float(lfCO2))
...
for name, pts in hits.items():
    ...
    fO2s = [p[2] for p in pts]
    fCO2s = [p[3] for p in pts]
    print(f"  T range (C): {min(Ts):.1f} to {max(Ts):.1f}")
    print(f"  pH range: {min(pHs):.2f} to {max(pHs):.2f}")
    print(f"  log10(fO2/bar) range: {min(fO2s):.1f} to {max(fO2s):.1f}")
    print(f"  log10(fCO2/bar) range: {min(fCO2s):.1f} to {max(fCO2s):.1f}")
    print("  first points:")
    for p in pts[:20]:
        print(
            f"    T={p[0]:.1f} C, pH={p[1]:.2f}, logfO2={p[2]:.1f}, logfCO2={p[3]:.1f}"
        )

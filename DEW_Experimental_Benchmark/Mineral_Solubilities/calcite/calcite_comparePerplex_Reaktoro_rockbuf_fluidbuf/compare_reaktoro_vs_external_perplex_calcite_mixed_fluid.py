"""
Direct Perple_X vs PerplexDEW mixed-fluid parity checks for calcite.
...
OUTPUT_CSV = SCRIPT_DIR / "perplex_calcite_mixed_fluid_parity_results.csv"

# Known low-temperature calcite parity behavior:
# At ~500-600 C, Reaktoro and Perple_X can differ by ~1 log unit in total dissolved
# Ca even when using similar DEW/GFSM ingredients. Targeted diagnostics showed that
# the offset is driven by different effective carbonate complexation in mixed fluids
# (not by a parser failure): apparent logK for Ca(HCO3)+ and CaCO3(aq) differ strongly
# at 500 C but converge at 700 C. This indicates a low-T acid-base/speciation-state
# mismatch between implementations and branch chemistry, while high-T behavior aligns.
LOW_T_CALCITE_DISCREPANCY_NOTE = (
    "Low-T calcite note: residual 500-600 C mismatch is expected from effective "
    "carbonate complexation/pH-state differences between Reaktoro and Perple_X "
    "mixed-fluid branches; high-T (700 C) equilibria converge closely."
)
...
def build_reaktoro_system(
    ...
    *,
    dh_model: str = "Davies",
    enable_hydrous_species_correction: bool = False,
):
    ...
    target_species = ["H2O(aq)", "H+(aq)", "OH-(aq)"] + list(cfg["species_map"].keys())
...
def solve_reaktoro_case_rock_buffered(
    system, case: Case, mineral_buffer_moles: float = 1000.0
) -> dict[str, float]:
    """Solve case with a large mineral inventory to approximate rock buffering."""
    cfg = MINERAL_CONFIGS[case.mineral]
    solver = EquilibriumSolver(system)
    conditions = EquilibriumConditions(system)
    conditions.temperature(case.t_c, "celsius")
    ...
    state.set("OH-(aq)", 1e-8, "mol")
    state.set(cfg["seed_species"], 1e-6, "mol")
    state.set(cfg["mineral_phase"], mineral_buffer_moles, "mol")
    ...
    result = solver.solve(state, conditions)
    if not result.succeeded():
        raise RuntimeError(f"PerplexDEW rock-buffered solve failed for {case.name}")
...
def configure_option_file(path: Path) -> None:
    ...
    # Continue output even when ver104 aqueous speciation fails in extreme cases.
    text = re.sub(r"(?m)^aq_error_ver104\s+\S+", "aq_error_ver104 F", text)
    if re.search(r"(?m)^aq_error_ver104\s+", text) is None:
        if not text.endswith("\n"):
            text += "\n"
        text += "aq_error_ver104 F\n"
    text = re.sub(
...
def parse_perplex_prn(
    prn_path: Path, mineral: str, verbose: bool = False
    ...
    # Aggregate species molalities from all immiscible fluid phases using mol% weights
    # For systems with 2 immiscible liquids, this produces a weighted average that
    # properly accounts for element distribution across both phases.
    values = {species: 0.0 for species in cfg["species_map"]}

    if verbose and nblocks > 1:
        print(f"  DEBUG: Multiple immiscible phases detected: {nblocks} blocks")
        print(f"         Phase weights (mol%): {weights}")
        print(f"         Weight sum (normalization): {wsum}")

    for species in values:
        accum = 0.0
        for idx, vals in enumerate(block_values):
            # Weight each phase's contribution by its molar fraction
            phase_molality = vals.get(species, 0.0)
            weighted_contrib = weights[idx] * phase_molality
            accum += weighted_contrib
        # Normalize by total fluid mol% to get system-scale molality
        values[species] = accum / wsum

    # Calculate total element amount by aggregating from all species in both phases
    # The element_coeffs specifies how many atoms of the element of interest
    # are in each species (e.g., for Calcite tracking Ca: Ca+2 has 1 Ca, CaCO3 has 1 Ca, etc.)
    total = 0.0
    for species, coeff in cfg["element_coeffs"].items():
        species_molality = values.get(species, 0.0)
        element_contribution = coeff * species_molality
        total += element_contribution
    values["total"] = total

    if verbose and nblocks > 1:
        print(
            f"         Aggregated total {cfg['mineral_component']}: {total:.6e} mol/kg"
        )
...
def run_perplex_case(
    ...
    try:
        ...
        return parse_perplex_prn(prn_path, case.mineral, verbose=keep_work_dir)
    finally:
        if keep_work_dir:
...
def compare_cases(
    ...
    print(f"Note: {LOW_T_CALCITE_DISCREPANCY_NOTE}")
    ...
    violations = []
    buffer_modes = [
        ("fluid_buffered", solve_reaktoro_case),
        ("rock_buffered", solve_reaktoro_case_rock_buffered),
    ]

    for case in CASE_MATRIX:
        print(
            f"\n[{case.mineral}] {case.name}: T={case.t_c:.1f} C  P={case.p_kbar:.1f} kbar  xCO2={case.xco2:.3f}"
        )
        case_error = ""
        try:
        ...
        for buffer_type, solve_fn in buffer_modes:
            rt_values = solve_fn(systems[case.mineral], case)
            for metric in metrics:
                ...
                if (
                    ...
                    violations.append((buffer_type, case.name, metric, delta))
            total_row = next(
                r
                for r in reversed(rows)
                if r["case"] == case.name
                and r["metric"] == "total"
                and r["buffer_type"] == buffer_type
            )
            if math.isfinite(px_values.get("total", float("nan"))):
                print(
                    f"  [{buffer_type}] total: PerplexDEW={rt_values['total']:.6e}  "
                    f"Perple_X={px_values['total']:.6e}  delta_log10={total_row['delta_log10']:+.3f}"
                )
            else:
                print(
                    f"  [{buffer_type}] total: PerplexDEW={rt_values['total']:.6e}  Perple_X=NaN"
                )
    ...
    if violations:
        ...
        for buf, case_name, metric, delta in violations:
            print(f"  [{buf}] {case_name}: {metric} delta_log10={delta:+.3f}")
        return 1
    return 0

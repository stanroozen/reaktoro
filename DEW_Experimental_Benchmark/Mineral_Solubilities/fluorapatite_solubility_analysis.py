...
MINERAL_CONFIG = {
    ...
    "mineral_name": "Fluorapatite",  # Name in database
    "mineral_formula": "Ca5(PO4)3F",  # Chemical formula
    ...
    "include_elements": ["Ca", "P", "F", "O", "H"],
    "aqueous_species": "",
    # File paths
    "csv_file": "fluorapatite_DEW_testset.csv",
    "output_prefix": "fluorapatite",  # Prefix for output files
    # Plot settings
    "plot_title": "Fluorapatite Solubility",
    "y_label": "Fluorapatite-equivalent molality (Ca/5, P/3, F; mol/kg-Hâ‚‚O)",
}
# ============================================================
...
def main():
    ...
    # Plot calculated curves for low pressures (Ca/5, P/3, F)
    for P_kbar in low_P_pressures:
        if (
            P_kbar not in element_curves["Ca"]
            or P_kbar not in element_curves["P"]
            or P_kbar not in element_curves["F"]
        ):
            continue
        ca_curve = element_curves["Ca"][P_kbar]
        p_curve = element_curves["P"][P_kbar]
        f_curve = element_curves["F"][P_kbar]

        ca_mol = np.array(ca_curve["molality"], dtype=float) / 5.0
        p_mol = np.array(p_curve["molality"], dtype=float) / 3.0
        f_mol = np.array(f_curve["molality"], dtype=float)

        ca_valid = np.isfinite(ca_mol)
        p_valid = np.isfinite(p_mol)
        f_valid = np.isfinite(f_mol)
        ...
        if np.any(ca_valid):
            ax1.plot(
                ...
                linewidth=2.0,
                linestyle="-",
                label=f"Calc Ca/5 P={P_kbar:.2f} kbar",
                zorder=5,
            )
        if np.any(p_valid):
            ax1.plot(
                ...
                linewidth=2.0,
                linestyle="--",
                label=f"Calc P/3 P={P_kbar:.2f} kbar",
                zorder=5,
            )
        if np.any(f_valid):
            ax1.plot(
                ...
                linewidth=2.0,
                linestyle=":",
                label=f"Calc F P={P_kbar:.2f} kbar",
                zorder=5,
            )

    # Plot Psat curves (Ca/5, P/3, F)
    if "Psat" in element_curves["Ca"]:
        curve_ca = element_curves["Ca"]["Psat"]
        curve_p = element_curves["P"]["Psat"]
        curve_f = element_curves["F"]["Psat"]
        ca_mol = np.array(curve_ca["molality"], dtype=float) / 5.0
        p_mol = np.array(curve_p["molality"], dtype=float) / 3.0
        f_mol = np.array(curve_f["molality"], dtype=float)
        ...
        if np.any(ca_valid):
            ax1.plot(
                curve_ca["T_C"][ca_valid],
                ca_mol[ca_valid],
                color="purple",
                linewidth=3.0,
                linestyle="-",
                label="Calc Ca/5 P=Psat",
                zorder=6,
                alpha=0.9,
            )
        if np.any(p_valid):
    ...
    # Plot calculated curves for high pressures (Ca/5, P/3, F)
    for P_kbar in high_P_all_pressures:
        if (
            P_kbar not in element_curves["Ca"]
            or P_kbar not in element_curves["P"]
    ...
    for _, row in exp_data.iterrows():
        ...
        if result.succeeded():
            try:
                aqprops = AqueousProps(state)
                molality = float(aqprops.speciesMolality(solute_species))
            except Exception:
                molality = np.nan
        else:
            molality = np.nan

"""
Quartz Solubility Analysis using Reaktoro with DEW2024 + Davies Activity Model
...
OUTPUT_PLOT_LOW = os.path.join(
    SCRIPT_DIR, "quartz_solubility_comparison_low_P_dew24_davies.png"
)
OUTPUT_PLOT_HIGH = os.path.join(
    SCRIPT_DIR, "quartz_solubility_comparison_high_P_dew24_davies.png"
)
RESIDUALS_PLOT = os.path.join(
    SCRIPT_DIR, "quartz_solubility_residuals_dew24_davies.png"
)
...
def build_system(dew_db, supcrt_db, water_config=None):
    ...
    try:
        ...
        aqueous.setActivityModel(ActivityModelDavies())
        eos_name = water_config.get("eos_model", "ZhangDuan2005")
        print(f"âœ“ DEW configured: EOS={eos_name}; phase activity=ActivityModelDavies")
    except Exception as e:
        print(f"Warning: Could not configure DEW params: {e}")
        aqueous.setActivityModel(ActivityModelDavies())
...
def main():
    print("=" * 80)
    print("Quartz Solubility Analysis - Reaktoro DEW2024 + Davies Activity Model")
    print("=" * 80)
    ...
    info_text = "DEW24 (species) + SUPCRTBL (quartz) + Davies Activity Model"
    ax1.text(
        0.02,
        0.98,
        info_text,

...
def parse_args():
    ...
    parser.add_argument(
        "--uncertainty-entity",
        default="q",
        help="Entity code in covariance JSON used for uncertainty propagation (default: q for Quartz).",
    )
    parser.add_argument(
        "--uncertainty-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        default="q",
        help="Mineral species code in mineral JSON used in equilibrium system (default: q for Quartz).",
    )
    parser.add_argument(
        "--quick-eval",
        action="store_true",
...
def main():
    ...
    outputs = output_paths(MINERAL_CONFIG, args.backend, dh_model)
    ...
    if len(psat_data) > 0:
        ...
        if quick_eval:
            T_psat_range = np.array([100.0, 300.0], dtype=float)
        else:
            T_psat_range = np.linspace(100, 374, n_curve_points)
    ...
    if (
        ...
        print(
            ...
            f"{len(pressures_for_curves)} pressure curves + Psat..."
        )
        ...
        for p in pressures_for_sampling:
            ...
            lo = np.nanquantile(arr, q_lo, axis=0)
            med = np.nanquantile(arr, q_med, axis=0)
            hi = np.nanquantile(arr, q_hi, axis=0)
            ...
            for interval in fan_intervals:
                lo_i, hi_i = interval_bounds_from_samples(arr, interval)
                solubility_curves[p][f"molality_ci{interval}_lo"] = lo_i
                solubility_curves[p][f"molality_ci{interval}_hi"] = hi_i

        arr_psat = np.array([s["Psat"] for s in all_sampled_curves], dtype=float)
        lo_psat = np.nanquantile(arr_psat, q_lo, axis=0)
        med_psat = np.nanquantile(arr_psat, q_med, axis=0)
        hi_psat = np.nanquantile(arr_psat, q_hi, axis=0)
        ...
        for interval in fan_intervals:
            lo_i, hi_i = interval_bounds_from_samples(arr_psat, interval)
    ...
    if uncertainty_enabled and show_predicted_point_overlay:
        ...
        low_overlay_data = pd.concat([low_P_data, low_psat_data], ignore_index=True)
        for _, row in low_overlay_data.iterrows():
            curve, curve_key = curve_for_experimental_row(row, low_P_pressures)
            if curve is None:
                continue
            t_c = float(row["T_C"])
    ...
    for P_kbar in high_P_all_pressures:
        ...
        for author in subset["reference"].unique():
            ...
            high_plot_positive_values.extend(
                _collect_positive_values(
                    author_subset["molality_m"].to_numpy(dtype=float)
                )
            )
    ...
    for P_kbar in high_P_all_pressures:
        ...
        if "molality_lo" in curve and "molality_hi" in curve:
            ...
            draw_uncertainty_fan(
                ax2, curve["T_C"], curve, P_to_color_high[P_kbar], zbase=4
            )
            high_plot_positive_values.extend(
                _collect_positive_values(
                    curve["molality_lo"][valid_band],
    ...
    if uncertainty_enabled and show_predicted_point_overlay:
        ...
        drew_label_high = False
        for _, row in high_P_all_data.iterrows():
            curve, curve_key = curve_for_experimental_row(row, high_P_all_pressures)
            if curve is None:
                continue
            t_c = float(row["T_C"])
            m_med = interp_curve_value(curve, t_c, "molality_med")
            ...
            if curve_key == "Psat":
                ...
                c = P_to_color_high.get(float(curve_key), "black")
            ...
            ax2.plot(
                ...
                alpha=1.0,
                zorder=14,
                label=(pred_label_high if not drew_label_high else None),
            )
            drew_label_high = True
    ...
    high_ymin, high_ymax = dynamic_log_ylim(
        np.asarray(high_plot_positive_values, dtype=float),
        fallback=(1e-4, 1e1),
    )
    ...
    ax2.legend(
        handles2 + fan_handles,
        labels2 + [h.get_label() for h in fan_handles],
        loc="center left",
        bbox_to_anchor=(1.02, 0.5),
        fontsize=9,
        framealpha=0.9,

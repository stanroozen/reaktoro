...
MINERAL_CONFIG = {
    ...
    # Plot settings
    "plot_title": "Enstatitefosterite Apparent Solubility",
    "y_label": "Apparent Enstatitefosterite Solubility (m_Si from Enstatite+Forsterite, mol/kg-H₂O)",
}
# ============================================================
...
def main():
    ...
    additional_minerals = MINERAL_CONFIG.get("additional_minerals", [])
    if additional_minerals:
        print(f"Additional minerals: {', '.join(additional_minerals)}")
    include_elements = MINERAL_CONFIG.get("include_elements")
    if include_elements:
        print(f"Aqueous elements: {', '.join(include_elements)}")
    print("=" * 80)
    ...
    if not os.path.exists(CSV_FILE):
        ...
        if len(exp_data) > 0:
            ...
            print(
                f"    Experimental molality range: {exp_data['molality_m'].min():.3g} - {exp_data['molality_m'].max():.3g} mol/kg-H₂O"
            )
    ...
    # =========================================================================
    # PLOT 2B: Element totals (Mg, Si) to assess incongruent dissolution
    # =========================================================================
    print("    Creating element total plots (Mg, Si)...")
    fig_el, axes_el = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
    element_labels = {
        "Mg": "m_Mg_tot",
        "Si": "m_Si_tot (apparent Enstatite+Forsterite)",
    }
    ...
    axes_el[0].set_title(
        "Enstatite+Forsterite system: apparent solubility uses total dissolved Si",
        fontsize=13,
        fontweight="bold",
        pad=12,
    )

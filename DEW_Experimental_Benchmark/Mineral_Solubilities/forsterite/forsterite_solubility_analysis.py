...
MINERAL_CONFIG = {
    ...
    "mineral_name": "Forsterite",  # Name in database
    "mineral_formula": "Mg2SiO4",  # Chemical formula
    ...
    "excluded_species": ["MgO(aq)"],
    "additional_minerals": [],
    # Aqueous species to include (besides water, H+, OH-)
    "aqueous_species": "",
    # File paths
    "csv_file": "forsterite_DEW_testset.csv",
    "output_prefix": "forsterite",  # Prefix for output files
    # Plot settings
    "plot_title": "Forsterite Solubility",
    "y_label": "Forsterite-equivalent molality (from Mg/2; mol/kg-H₂O)",
}
# ============================================================
...
def main():
    ...
    # Plot calculated curves for high pressures (Mg/2 and Si-based Fo-equivalent)
    y_positive = []

    # Stoichiometric comparison lines based on element totals
    for P_kbar in high_P_all_pressures:
        if P_kbar not in element_curves["Mg"] or P_kbar not in element_curves["Si"]:
            continue
        mg_curve = element_curves["Mg"][P_kbar]
        si_curve = element_curves["Si"][P_kbar]

        mg_mol = np.array(mg_curve["molality"], dtype=float) / 2.0
        si_mol = np.array(si_curve["molality"], dtype=float)

        mg_valid = np.isfinite(mg_mol) & (mg_mol > 0)
        si_valid = np.isfinite(si_mol) & (si_mol > 0)

        if np.any(mg_valid):
            ax2.plot(
                mg_curve["T_C"][mg_valid],
                mg_mol[mg_valid],
                color=P_to_color_high[P_kbar],
                linewidth=2.0,
                linestyle="--",
                alpha=0.8,
                label=f"Fo from Mg/2 P={P_kbar:.2f} kbar",
                zorder=12,
            )
            y_positive.extend(mg_mol[mg_valid].tolist())
        ...
        if np.any(si_valid):
            ax2.plot(
                ...
                linestyle=":",
                alpha=0.8,
                label=f"Fo from Si P={P_kbar:.2f} kbar",
                zorder=12,
            )
            y_positive.extend(si_mol[si_valid].tolist())
    ...
    # =========================================================================
    # PLOT 2B: Element totals (Mg, Si)
    # =========================================================================
    print("    Creating element total plots (Mg, Si)...")
    fig_el, axes_el = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
    element_labels = {
        "Mg": "m_Mg_tot",
        "Si": "m_Si_tot",
    }
    for ax_el, elem in zip(axes_el, ("Mg", "Si")):
        for P_kbar in high_P_all_pressures:
        ...
        ax_el.set_ylabel(
            f"{element_labels[elem]} (mol/kg-H₂O)", fontsize=12, fontweight="bold"
        )
        ax_el.grid(True, which="both", alpha=0.3, linestyle="--")

    axes_el[-1].set_xlabel("Temperature (°C)", fontsize=12, fontweight="bold")
    axes_el[0].set_title(
        "Forsterite stoichiometry check: m_Fo from Mg/2 vs m_Fo from Si",
        fontsize=13,
        fontweight="bold",
        pad=12,
    )

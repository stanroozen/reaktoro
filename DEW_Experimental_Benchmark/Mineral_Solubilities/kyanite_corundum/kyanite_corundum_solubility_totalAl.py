...
MINERAL_CONFIG = {
    ...
    "include_elements": ["Al", "Si", "O", "H"],
    "aqueous_species": "",
    "additional_minerals": ["Kyanite"],
    # File paths
    "csv_file": "kyanite_corundum_DEW_testset_totalAl.csv",
    "output_prefix": "corundum_kyanite_TotalAl",  # Prefix for output files
    # Plot settings
    "plot_title": "Corundum + Kyanite Solubility",
    "y_label": "Total Al molality (mol/kg-H₂O)",
}
# ============================================================
...
def build_system(dew_db, supcrt_db, mineral_config, water_config=None):
    ...
    for extra in additional_minerals:
        try:
            combined_db.addSpecies(supcrt_db.species(extra))
            additional_mineral_phases.append(MineralPhase(extra))
        except Exception as e:
            print(f"    WARNING: Could not add mineral '{extra}': {e}")
...
def main():
    ...
    if not os.path.exists(CSV_FILE):
        ...
        exp_data = pd.DataFrame(
            columns=["T_C", "P_kbar", "molality_m", "reference", "experiment_type"]
        )
    else:
    ...
    if len(pressures_for_curves) == 0:
        pressures_for_curves = DEFAULT_PRESSURES
    ...
    for extra_mineral in MINERAL_CONFIG.get("additional_minerals", []):
        state_psat.set(extra_mineral, 10.0, "mol")
    ...
    # Plot calculated curves for high pressures (total Al)
    for P_kbar in high_P_all_pressures:
        ...
        ax2.plot(
            ...
            linewidth=2.0,
            linestyle="-",
            label=f"Calc Al_tot P={P_kbar:.2f} kbar",
            zorder=5,
        )

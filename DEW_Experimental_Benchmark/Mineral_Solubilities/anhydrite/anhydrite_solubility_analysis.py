...
try:
    ...
    # Script is 3 levels deep: reaktoro/DEW_Experimental_Benchmark/Mineral_Solubilities/anhydrite/
    _d0 = os.path.dirname(os.path.abspath(__file__))  # anhydrite/
...
# Script is 3 levels deep: reaktoro/DEW_Experimental_Benchmark/Mineral_Solubilities/anhydrite/
_ANHYDRITE_DIR = SCRIPT_DIR  # .../anhydrite/
_MINERAL_SOL_DIR = os.path.dirname(_ANHYDRITE_DIR)  # .../Mineral_Solubilities/
_DEW_BENCHMARK_DIR = os.path.dirname(
    _MINERAL_SOL_DIR
)  # .../DEW_Experimental_Benchmark/
_ROOT_DIR = os.path.dirname(_DEW_BENCHMARK_DIR)  # reaktoro root
...
MINERAL_CONFIG = {
    ...
    "aqueous_species": "",
    # File paths
    "csv_file": "anhydrite_DEW_testset.csv",
    "output_prefix": "anhydrite",  # Prefix for output files
    # Plot settings
    "plot_title": "Anhydrite Solubility",
...
def load_experimental_data(csv_file):
    ...
    required = ["t_c", "p_kbar", "molality_m", "reference", "experiment_type"]
...
def main():
    ...
    _state_set(state_psat, "H2O(aq)", 1.0, "kg")
    _state_set(state_psat, "H+(aq)", 1e-8, "mol")
    _state_set(state_psat, "OH-(aq)", 1e-8, "mol")
    _state_set(state_psat, solute_species, 1e-6, "mol")
    _state_set(state_psat, mineral_name, 10.0, "mol")
    ...
    for P_kbar in high_P_all_pressures:
        ...
        for author in subset["reference"].unique():
            ...
            ranged_mask = (
                author_subset["molality_max"].notna()
                & author_subset["molality_min"].notna()
                & (author_subset["molality_max"] > author_subset["molality_min"])
            )
            non_ranged = author_subset[~ranged_mask]
            if len(non_ranged) > 0:
                ax2.scatter(
                    non_ranged["T_C"],
            ...
            ranged = author_subset[ranged_mask]
            if len(ranged) > 0:
    ...
    for P_kbar in high_P_all_pressures:
        if P_kbar not in element_curves["Ca"]:
            continue
        ca_curve = element_curves["Ca"][P_kbar]
        ...
        if np.any(ca_valid):
            ax2.plot(
                ...
                linewidth=2.0,
                linestyle="-",
                label=f"Calc Ca P={P_kbar:.2f} kbar",
                zorder=5,
            )
    ...
    for _, row in exp_data.iterrows():
        ...
        for extra_mineral in MINERAL_CONFIG.get("additional_minerals", []):
            _state_set(state, extra_mineral, 10.0, "mol")

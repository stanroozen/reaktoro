...
# Use a non-interactive backend to ensure plots save in headless sessions.
import matplotlib
...
try:
    ...
    DEW_DIR = os.path.dirname(BENCHMARK_DIR)
    ROOT_DIR = os.path.dirname(DEW_DIR)  # Go up 3 levels to reach reaktoro root
...
MINERAL_CONFIG = {
    ...
    "aqueous_species": "O2(aq) HS-(aq) H2S(aq) HSO3-(aq) SO3-2(aq) S2O3-2(aq) S2-2(aq) SO4-2(aq) HSO4-(aq)",
    # File paths
...
# Include sulfur-bearing gases to keep sulfur redox partitioning responsive
# under fixed oxygen fugacity constraints.
...
def buffer_fugacity_bar(buffer_name, T_C, P_bar):
    ...
    if value is None and buffer_supcrtbl is not None:
        try:
            if _BUFFER_SUPCRT_DB is None:
                _BUFFER_SUPCRT_DB = Database.fromFile(SUPCRTBL_DATABASE_FILE)
            return float(
                buffer_supcrtbl.buffer_fugacity_bar(
                    "fO2", name, T_C, P_bar, db=_BUFFER_SUPCRT_DB
                )
            )
        except Exception:
            value = None
...
def seed_fugacity_species(state, species_name, amount=1e-8):
    ...
    try:
        ...
        try:
            state.set(species_name, autodiff.real(float(amount)), "mol")
        except Exception:
            return
    except Exception:
        return
...
def set_state_amount(state, species_name, amount, unit):
    """Set state amount with compatibility for local reaktoro4py typed API."""
    try:
...
def build_system(dew_db, supcrt_db, mineral_config, water_config=None):
    ...
    gas_phase = None
    available_gas_species = []
    for gas_species_name in GAS_SPECIES:
        try:
            combined_db.addSpecies(supcrt_db.species(gas_species_name))
            available_gas_species.append(gas_species_name)
        except Exception:
            print(f"    WARNING: Gas species not found in SUPCRTBL: {gas_species_name}")

    if available_gas_species:
        gas_phase = GaseousPhase(" ".join(available_gas_species))
...
def main():
    ...
    for P_kbar in pressures_for_curves:
        ...
        for buffer_name in curve_buffers:
            ...
            set_state_amount(state, solute_species, 1e-6, "mol")
    ...
    for buffer_name in curve_buffers:
        ...
        set_state_amount(state_psat, "H2O(aq)", 55.5, "mol")
        set_state_amount(state_psat, "H+(aq)", 1e-8, "mol")
        set_state_amount(state_psat, "OH-(aq)", 1e-8, "mol")
        set_state_amount(state_psat, solute_species, 1e-6, "mol")
        set_state_amount(state_psat, mineral_name, 10.0, "mol")
    ...
    for _, row in exp_data.iterrows():
        ...
        for extra_mineral in MINERAL_CONFIG.get("additional_minerals", []):
            set_state_amount(state, extra_mineral, 10.0, "mol")

...
MINERAL_CONFIG = {
    ...
    "mineral_name": "Portlandite",  # Name in database
    "mineral_formula": "Ca(OH)2",  # Chemical formula
    ...
    "include_elements": ["Ca", "O", "H"],
    "aqueous_species": "",
    # File paths
    "csv_file": "portlandite_DEW_testset.csv",
    "output_prefix": "portlandite",  # Prefix for output files
    # Plot settings
    "plot_title": "Portlandite Solubility",
...
def remap_species_elements(species, target_db):
    """Return a copy of species using element definitions from target_db."""
    mapped = []
    for element, coeff in species.elements():
        try:
            mapped_element = target_db.element(element.symbol())
        except Exception:
            mapped_element = element
        mapped.append((mapped_element, coeff))
    return species.clone().withElements(mapped)


def build_system(dew_db, mineral_db, mineral_config, water_config=None):
    """Build ChemicalSystem combining DEW aqueous species with a cemdata18 mineral.

    Args:
        dew_db: DEW database (aqueous species + water)
        mineral_db: ThermoFun database (cemdata18 mineral)
    ...
    aqueous_source_db = dew_db

    try:
        combined_db.addSpecies(mineral_species)
    except Exception as e:
        print(f"    WARNING: Could not merge mineral into DEW database: {e}")
        print("    Retrying merge using DEW element definitions for Portlandite.")
        combined_db = Database(dew_db.species())
        mineral_species_dew = remap_species_elements(mineral_species, dew_db)
        try:
            combined_db.addSpecies(mineral_species_dew)
            print("    âœ“ Portlandite merged into DEW database using DEW elements.")
        except Exception as e2:
            raise RuntimeError(
                "Failed to merge Portlandite into DEW database while keeping DEW aqueous species."
            ) from e2

    try:
        combined_db.species(mineral_name)
    except Exception:
        matches = [
            sp.name() for sp in combined_db.species() if mineral_name in sp.name()
        ]
        if matches:
            mineral_name = matches[0]
            mineral_config["mineral_name"] = mineral_name
            print(
                f"    WARNING: Using mineral name '{mineral_name}' from combined database."
            )
        else:
            raise RuntimeError(
                f"Portlandite not found in combined database (searched '{mineral_name}')."
            )
    ...
    if include_elements:
        aqueous_species_list = aqueous_species_by_elements(
            aqueous_source_db, include_elements
        )
        aqueous_species_list = sorted(set(aqueous_species_list))
        target_element = mineral_config.get("target_element")
        if target_element:
            mineral_config["_element_species_map"] = {
                target_element: element_species_coeffs(
                    aqueous_source_db, aqueous_species_list, target_element
                )
            }
        validate_aqueous_species(aqueous_source_db, aqueous_species_list)
        ...
        if target_element:
            mineral_config["_element_species_map"] = {
                target_element: element_species_coeffs(
                    aqueous_source_db, species_names, target_element
                )
            }
        validate_aqueous_species(aqueous_source_db, aqueous_species_str)
...
def main():
    ...
    try:
        mineral_db = ThermoFunDatabase("cemdata18")
        print("    Successfully loaded ThermoFun mineral database: cemdata18")
    except Exception as e:
        print(f"    ERROR: Failed to load ThermoFun database: {e}")
        raise

    system = build_system(dew_db, mineral_db, MINERAL_CONFIG)
    ...
    # Plot calculated curves for high pressures (total Si)
    for P_kbar in high_P_all_pressures:
        ...
        ax2.plot(
            ...
            linewidth=2.0,
            linestyle="-",
            label=f"Calc Si_tot P={P_kbar:.2f} kbar",
            zorder=5,
        )
    ...
    info_text = (
        f"DEW24 (aqueous) + cemdata18 ({mineral_name}) + Zhang-Duan 2005 EOS (Hâ‚‚O)\n"
        "Note: Portlandite $C_p$ not calibrated to experimental temperatures"
    )
    ax2.text(
        0.02,
        0.98,

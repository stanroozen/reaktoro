...
DEFAULT_SUPCRTBL_DATABASE_FILE = os.path.join(
    ROOT_DIR, "embedded", "databases", "reaktoro", "supcrtbl.yaml"
)
...
DEFAULT_INITIAL_SOLUTE_MOL = 1e-7

# Legacy deterministic compatibility settings (from brucite_solubility_analysis.py)
LEGACY_T_MIN, LEGACY_T_MAX = 150, 550
LEGACY_N_POINTS = 100
LEGACY_INITIAL_SOLUTE_MOL = 1e-6
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Brucite solubility benchmark with optional legacy deterministic "
            "compatibility mode."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use legacy deterministic settings exactly (SUPCRTBL Brucite, DEW backend, "
            "legacy grid and initial dissolved Mg seed)."
        ),
    )
    parser.add_argument(
        "--legacy-supcrtbl-db",
        default=DEFAULT_SUPCRTBL_DATABASE_FILE,
        help="Path to SUPCRTBL database used in compatibility mode.",
    )
    parser.add_argument(
        "--legacy-mineral-name",
        default="Brucite",
        help="Mineral species name in SUPCRTBL for compatibility mode.",
    )
...
def evaluate_solubility_curve(
    system,
    t_range_c,
    p_bar,
    mineral_species_name,
    initial_solute_mol=DEFAULT_INITIAL_SOLUTE_MOL,
):
    ...
    state.set(MINERAL_CONFIG["solute_species"], to_real(initial_solute_mol), "mol")
...
def evaluate_curves_on_fixed_grids(
    ...
    for p_kbar, t_range in pressure_grids.items():
        ...
        curves[p_kbar] = evaluate_solubility_curve(
            ...
            p_bar,
            mineral_species_name,
            initial_solute_mol=initial_solute_mol,
        )
    return curves
...
def output_paths(backend_name, dh_model=None, compat_legacy=False):
    prefix = MINERAL_CONFIG["output_prefix"]
    tag = str(backend_name)
    if tag.lower() == "perplexdew" and dh_model:
        tag = f"{tag}_{dh_model}"
    if compat_legacy:
        tag = f"{tag}_legacycompat"
    tag += "_uncertainty"
    family = "legacy_dew24" if compat_legacy else "hp_dew24"
    return {
        "main_plot": os.path.join(
            SCRIPT_DIR, f"{prefix}_solubility_{family}_{tag}.png"
        ),
        "residuals_plot": os.path.join(
            SCRIPT_DIR, f"{prefix}_residuals_{family}_{tag}.png"
        ),
        "residuals_csv": os.path.join(
            SCRIPT_DIR, f"{prefix}_residuals_{family}_{tag}.csv"
        ),
        "curves_csv": os.path.join(SCRIPT_DIR, f"{prefix}_curves_{family}_{tag}.csv"),
        "uncertainty_csv": os.path.join(
            SCRIPT_DIR, f"{prefix}_uncertainty_{family}_{tag}.csv"
        ),
    }
...
def main():
    args = parse_args()
    compat_legacy = bool(args.compat_legacy)
    model_backend = "DEW" if compat_legacy else str(args.backend)
    ...
    legacy_supcrtbl_db = args.legacy_supcrtbl_db
    legacy_mineral_name = str(args.legacy_mineral_name)
    outputs = output_paths(model_backend, dh_model, compat_legacy=compat_legacy)
    n_curve_points = (
        quick_npoints
        if quick_eval
        else (LEGACY_N_POINTS if compat_legacy else N_POINTS)
    )
    initial_solute_mol = (
        LEGACY_INITIAL_SOLUTE_MOL if compat_legacy else DEFAULT_INITIAL_SOLUTE_MOL
    )

    print("=" * 80)
    if compat_legacy:
        print("Brucite Solubility Analysis — LEGACY COMPAT MODE")
        print(
            f"Mineral: {MINERAL_CONFIG['mineral_formula']} (SUPCRTBL species: {legacy_mineral_name})"
        )
    else:
    ...
    if compat_legacy:
        print("Mode: legacy deterministic compatibility")
    ...
    if uncertainty_enabled:
        ...
        if compat_legacy:
            print(
                "Uncertainty source: disabled in legacy compatibility mode (SUPCRTBL has no tc-ds62 covariance mapping)."
            )
    ...
    if compat_legacy:
        if not os.path.exists(legacy_supcrtbl_db):
            raise FileNotFoundError(
                f"SUPCRTBL database not found: {legacy_supcrtbl_db}"
            )
        base_mineral_db = Database.fromFile(legacy_supcrtbl_db)
        base_mineral_species = base_mineral_db.species(legacy_mineral_name)
        mineral_species_name = legacy_mineral_name
        print(f"    SUPCRTBL mineral '{legacy_mineral_name}' loaded: OK")
    ...
    for p_kbar in pressures_for_curves:
        ...
        if len(exp_at_P) > 0:
            ...
            if compat_legacy:
                T_min = (
                    max(25.0, T_min_cat - 0.05 * T_span)
                    if T_span > 0
                    else T_min_cat - 50.0
                )
                T_max = (
                    min(1000.0, T_max_cat + 0.05 * T_span)
                    if T_span > 0
                    else T_max_cat + 50.0
                )
            else:
            ...
            if compat_legacy:
                T_min, T_max = float(LEGACY_T_MIN), float(LEGACY_T_MAX)
            else:
    ...
    if compat_legacy:
        print(
            f"  Mineral      : Brucite Mg(OH)2 | SUPCRTBL species '{legacy_mineral_name}'"
        )
    else:
        print(
    ...
    print(f"  Compat mode  : {compat_legacy}")
    print(f"  Seed state   : initial dissolved Mg = {initial_solute_mol:.1e} mol")

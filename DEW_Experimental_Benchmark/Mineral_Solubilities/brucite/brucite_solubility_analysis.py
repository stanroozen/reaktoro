...
try:
    ...
    # Script is 3 levels deep: reaktoro/DEW_Experimental_Benchmark/Mineral_Solubilities/brucite/
    _d0 = os.path.dirname(os.path.abspath(__file__))  # brucite/
    _d1 = os.path.dirname(_d0)  # Mineral_Solubilities/
    _d2 = os.path.dirname(_d1)  # DEW_Experimental_Benchmark/
    _ROOT_DIR_IMPORT = os.path.dirname(_d2)  # reaktoro/ root
    PYD_DIR = os.path.join(_ROOT_DIR_IMPORT, "build", "Reaktoro", "Release")
...
import autodiff  # must come after sys.path is patched with PYD_DIR
...
# Path to SUPCRTBL database (embedded in repo)
# Script is 3 levels deep: reaktoro/DEW_Experimental_Benchmark/Mineral_Solubilities/brucite/
_BRUCITE_DIR = SCRIPT_DIR
_MINERAL_SOL_DIR = os.path.dirname(_BRUCITE_DIR)
...
MINERAL_CONFIG = {
    ...
    "mineral_name": "Brucite",  # Name in database
    "mineral_formula": "Mg(OH)2",  # Chemical formula
    "target_element": "Mg",  # Element to report total dissolved molality
    "solute_species": "Mg+2(aq)",  # Primary aqueous species
    # Aqueous species to include (besides water, H+, OH-)
    "aqueous_species": "MgOH+(aq)",
    # File paths
    "csv_file": "brucite_DEW_testset.csv",
    "output_prefix": "brucite",  # Prefix for output files
    # Plot settings
    "plot_title": "Brucite Solubility",
    "y_label": "Brucite Solubility (mol/kg-Hâ‚‚O)",
}
# ============================================================
...
def build_system(dew_db, supcrt_db, mineral_config, water_config=None):
    ...
    # Cache element-bearing species with stoichiometric coefficients
...
def load_experimental_data(csv_file):
    ...
    if {"T_C", "P_kbar", "molality_m"}.issubset(df.columns):
        ...
        # Find temperature and molality columns robustly (column names may have encoding artefacts)
        t_col = next(
            (c for c in df.columns if c.startswith("T ") and "C" in c),
            None,
        )
        m_col = next(
            (c for c in df.columns if c.lower().startswith("molality")),
            None,
        )
        if t_col is None or m_col is None:
            raise KeyError(
                f"Could not identify T or Molality columns in {csv_file}. Columns: {list(df.columns)}"
            )
        p_col = "P (bar)"
...
def main():
    ...
    print("    WARNING: MgO_aq excluded (not a real aqueous species).")
    ...
    try:
        ...
        print(
            f"    Successfully loaded SUPCRTBL mineral database from {SUPCRTBL_DATABASE_FILE}"
        )
    except Exception as e:
    ...
    for P_kbar in pressures_for_curves:
        ...
        def _state_set(st, name, amount, unit):
            try:
                st.set(name, float(amount), unit)
            except TypeError:
                st.set(name, autodiff.real(float(amount)), unit)

        state = ChemicalState(system)
        _state_set(state, "H2O(aq)", 1.0, "kg")
        _state_set(state, "H+(aq)", 1e-8, "mol")
        _state_set(state, "OH-(aq)", 1e-8, "mol")
        _state_set(state, solute_species, 1e-6, "mol")
        _state_set(state, mineral_name, 10.0, "mol")
    ...
    for P_kbar in high_P_all_pressures:
        ...
        for reversal in subset["reversal"].unique():
            ...
            ax2.scatter(
                ...
                edgecolors="black",
                linewidths=0.4,
                label=f"Exp P={P_kbar:.2f} kbar ({reversal})",
                zorder=10,
            )
    ...
    if len(pressures_for_curves) > 0:
        ...
        if spec_P_kbar in solubility_curves:
            ...
            _state_set(state_spec, "H2O(aq)", 1.0, "kg")
            _state_set(state_spec, "H+(aq)", 1e-8, "mol")
            _state_set(state_spec, "OH-(aq)", 1e-8, "mol")
            _state_set(state_spec, solute_species, 1e-6, "mol")
            _state_set(state_spec, mineral_name, 10.0, "mol")

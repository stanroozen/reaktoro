...
MINERAL_CONFIG = {
    ...
    "mineral_name": "Rutile",  # Name in database
    "mineral_formula": "TiO2",  # Chemical formula
    "target_element": "Ti",  # Element to report total dissolved molality
    "solute_species": "TIO2_aq",  # Primary aqueous species (DEW name)
    # Aqueous species to include (besides water, H+, OH-)
    "include_elements": ["Ti", "O", "H"],
    "aqueous_species": "",
    # File paths
    "csv_file": "rutile_DEW_testset.csv",
    "output_prefix": "rutile",  # Prefix for output files
    # Plot settings
    "plot_title": "Rutile Solubility",
    "y_label": "Total Ti molality (mol/kg-Hâ‚‚O)",
}
# ============================================================
...
def build_system(dew_db, supcrt_db, mineral_config, water_config=None):
    ...
    species_list = aqueous_species_str.split()
    missing = []
    for name in list(species_list):
        try:
            dew_db.species(name)
        except Exception:
            missing.append(name)
    ...
    if missing:
        print(
            "    WARNING: Removing missing aqueous species from system: "
            + ", ".join(missing)
        )
        species_list = [name for name in species_list if name not in missing]
        if solute in missing:
            mineral_config["solute_species"] = ""
            solute = ""
            print("    WARNING: Solute species missing; continuing without it.")
        aqueous_species_str = " ".join(species_list)
    ...
    if target_element:
        mineral_config["_element_species_map"] = {
            target_element: element_species_coeffs(dew_db, species_list, target_element)
        }
...
def main():
    ...
    # Plot calculated curves for high pressures (Ti)
    for P_kbar in high_P_all_pressures:
        if P_kbar not in element_curves["Ti"]:
            continue
        ti_curve = element_curves["Ti"][P_kbar]

        ti_mol = np.array(ti_curve["molality"], dtype=float)
        ti_valid = np.isfinite(ti_mol)

        if np.any(ti_valid):
            ax2.plot(
                ti_curve["T_C"][ti_valid],
                ti_mol[ti_valid],
                color=P_to_color_high[P_kbar],
                linewidth=2.0,
                linestyle="-",
                label=f"Calc Ti P={P_kbar:.2f} kbar",
                zorder=5,
            )

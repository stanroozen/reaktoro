...
MINERAL_CONFIG = {
    ...
    "mineral_name": "Fluorite",  # Name in database
    "mineral_formula": "CaF2",  # Chemical formula
    ...
    "include_elements": ["Ca", "F", "O", "H"],
    "aqueous_species": "",
    # File paths
    "csv_file": "fluorite_DEW_testset.csv",
    "output_prefix": "fluorite",  # Prefix for output files
    # Plot settings
    "plot_title": "Fluorite Solubility",
    "y_label": "Fluorite-equivalent molality (Ca, F/2; mol/kg-Hâ‚‚O)",
}
# ============================================================
...
def main():
    ...
    # Plot calculated curves for high pressures (Ca, F/2)
    for P_kbar in high_P_all_pressures:
        if P_kbar not in element_curves["Ca"] or P_kbar not in element_curves["F"]:
            continue
        ca_curve = element_curves["Ca"][P_kbar]
        f_curve = element_curves["F"][P_kbar]

        ca_mol = np.array(ca_curve["molality"], dtype=float)
        f_mol = np.array(f_curve["molality"], dtype=float) / 2.0
        ...
        if np.any(f_valid):
            ax2.plot(
                ...
                linewidth=2.0,
                linestyle="--",
                label=f"Calc F/2 P={P_kbar:.2f} kbar",
                zorder=5,
            )

...
DIAG_DIR = os.path.abspath(os.path.join(BASE, ".."))

pairs = [
    (
        os.path.join(BASE, "CHNOSZ_Pourbaix_Fe.png"),
        os.path.join(DIAG_DIR, "Section2_PredominancePlot_Fe_Pourbaix.png"),
        "Fe Pourbaix",
    ),
    (
        os.path.join(BASE, "CHNOSZ_Mosaic_Fe.png"),
        os.path.join(DIAG_DIR, "Section5_MosaicPlot_Fe.png"),
        "Fe Mosaic",
    ),
]
...
out_panel = os.path.join(BASE, "CHNOSZ_vs_Reaktoro_panels.png")
fig.suptitle("CHNOSZ vs Reaktoro Diagram Comparison", fontsize=16)
fig.savefig(out_panel, dpi=180)
plt.close(fig)

out_txt = os.path.join(BASE, "CHNOSZ_vs_Reaktoro_metrics.txt")

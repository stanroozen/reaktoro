...
try:
    ...
    try:
        ...
        loaded_from = os.path.dirname(getattr(local_mod, "__file__", ""))
    except ModuleNotFoundError:
        pass
...
def build_system(
    ...
    # Species names must match the active DEW database in this build.
...
def main():
    ...
    if len(exp_data) > 0:
        ...
        psat_data = pd.DataFrame(columns=["T_C", "P_kbar", "experiment_id", "is_psat"])
        non_psat_data = pd.DataFrame(
            columns=["T_C", "P_kbar", "experiment_id", "is_psat"]
        )

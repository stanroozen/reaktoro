...
try:
    ...
    if _loaded_from is None:
        _root_dir = os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR)))
        _pyd_candidates = [
            os.path.join(_root_dir, "build", "Reaktoro", "Release"),
        ]
        for _pyd_dir in _pyd_candidates:
            ...
            _loaded_from = os.path.dirname(getattr(_local_mod, "__file__", _pyd_dir))
            break
    if _loaded_from is None:
...
def load_experimental_data(csv_file):
    ...
    if not os.path.exists(csv_file):
        return pd.DataFrame(
            columns=["xco2", "molality_m", "P_kbar", "T_C", "reference"]
        )
...
def main():
    ...
    print(
        f"Gas EOS   : H2O-CO2 water activity â€” Zhang-Duan 2009 (GFSM, separate pass)"
    )
    ...
    print(
        f"Si species: SiO2(aq), HSiO3-(aq), Si2O4(aq), Si3O6(aq) (no Si-C species in DEW2024)"
    )
    print("=" * 80)
    ...
    if len(exp_df) == 0:
        print(f"    WARNING: Experimental data file not found or empty: {CSV_FILE}")
    ...
    for P_kbar in MODEL_PRESSURES_KBAR:
        ...
        print(
            f"      mâ‚€(SiO2(aq))={m0['SiO2(aq)']:.4f}  mâ‚€(Si2O4(aq))={m0['Si2O4(aq)']:.4f}  "
            f"mâ‚€(Si3O6(aq))={m0['Si3O6(aq)']:.4e}  mâ‚€(HSiO3-(aq))={m0['HSiO3-(aq)']:.4e}  "

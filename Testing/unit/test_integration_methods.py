...
try:
    ...
    try:
        ...
        pytest.skip(f"reaktoro not available: {e}", allow_module_level=True)

# Load truth data from Excel CSV
truth_file = Path("Reaktoro/Extensions/DEW/tests/reactionTesttruth.csv")
if not truth_file.exists():
    # Fallback mini dataset keeps this regression runnable in lean workspaces.
    df_truth = pd.DataFrame(
        {
            "T_C": [200.0, 300.0, 400.0],
            "P_kb": [1.0, 2.0, 5.0],
        }
    )
else:
